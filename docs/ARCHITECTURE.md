...
src/data/                    bodies.json, credits.json (image sources + licences, #37 and planet textures), schema.ts (zod), index.ts (lookups), solarDictionary.ts (dictionary + hero adapter),
...
- **`sources`**: as for the moons (title, credit, short credit for the card, landing `url`, download `file`,
  ...
  the Dawn/DLR mosaic (Vesta, kept for #23).
...
- Output: JPEG q82 (Earth's day and night maps 4096 px, the rest 2048 or less), about 4 MB in all. `pnpm gen:surfaces
