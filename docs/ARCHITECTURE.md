...
src/store/                   sim.ts, navigation.ts, flight.ts, scale.ts, lighting.ts, spin.ts, trails.ts, light.ts, tour.ts, simSearch.ts (URL schema), urlSync.ts
src/features/                hero/, solarDictionary/, solarSystem/ (index.tsx, scene/, bodies/, camera/, frame/, labels/, lighting/, light/, rings/, tours/, ui/),
...
off by default, as `orbitNames=true` while on; `frame=<id>` while a body is held still (#31); `tour=<id>&stop=<n>` (and `autoplay=true`) while a guided tour runs (#28). Defaults (overview, home shot `0_45_1`, `warp=1`, a switch that is on)
