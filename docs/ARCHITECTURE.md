...
src/sim/                     pure simulation code (no React, no three.js objects): units.ts, time.ts, clock.ts, kepler.ts, positions.ts,
                             rotation.ts, scale.ts (the scale engine, see Scale), lighting.ts (sunlight and eclipses, see Lighting),
                             index.ts (barrel, import from "@/sim");
...
src/store/                   sim.ts (the simulation store), navigation.ts (selection and the camera's view, a slice of
                             the simulation store, see Navigation), scale.ts (the active scale, see Scale), lighting.ts
                             ("always lit", see Lighting), simSearch.ts (zod schema of the
...
src/features/solarSystem/    the 3D solar system: index.tsx (page), scene/ (Scene, SimClock, ScaleSync, simFrame, Markers, useThrottledSimTime),
                             ...
                             framing, pose, profiles, input, debugHandle), lighting/ (sunlightShader, bodyLighting,
                             SunlitMaterial; see Lighting), ui/ (TimeControls, SceneToggles, FocusPicker,
...
interface Orbit {
	...
	periodDays: number // sidereal orbital period (of the mean longitude when the orbit precesses)
	...
	precession?: { nodeDegPerDay: number; argPeriapsisDegPerDay: number } // secular turning of the orbit (the Moon only)
}
...
- Hand-curated corrections are made in `data/ourDB.json` itself (it is "export + curated"): the Sun's mass exponent (30)
  ...
  typos, and the Moon's secular precession (`nodePrecessionDegPerDay` -0.052992, `argPeriapsisPrecessionDegPerDay`
  0.1643586: Meeus ch. 47 rates, the node's referred to the fixed J2000 equinox; with them `sideralOrbit` is the true
  sidereal month 27.321661 d; issue #22): Phobos' rotation (7.6534 h, was 0.7653), Iapetus' rotation (1903.92 h, synchronous), Metis' period (0.29478 d),
...
- Precession (#22): an orbit with `precession` turns: node and argument of periapsis advance linearly from the epoch and
  the mean anomaly runs slower by their sum (P stays the sidereal period of the mean longitude, so the Moon's anomaly
  runs at the anomalistic month). `orbitAt(orbit, jd, out)` returns the fixed ellipse the body is on at `jd` (no
  precession of its own, same mean anomaly at `jd`); `propagate` uses it, orbit lines resample from it. Only the Moon
  has it: without it the Moon's perigee and node freeze at J2000 and it drifts 16 deg off the ephemeris by 2045, which
  puts eclipses on the wrong dates; with it the Moon stays within 2.4 deg (evection and variation are not modelled) and
  real eclipses happen within about three hours of the real ones (`precession.test.ts`, `lighting.test.ts`).
...
- Accuracy: the eight planets agree with astronomy-engine to < 0.2 deg / 0.15 percent over J2000 +- 2000 days
  ...
  a 0.3 deg / 0.3 percent gate; `kepler.test.ts` checks the propagator with its own element table); the Moon to 2.5 deg
  from 2000 to 2045 (`precession.test.ts`) and the Galilean moons to a few degrees. Everything else has fictitious phases
  (see Data model).
...
## Lighting (`src/sim/lighting.ts`, `src/features/solarSystem/lighting/`, `src/store/lighting.ts`; issue #22)

The single authority on how anything in the scene is lit. The Sun (the root body) is the only light source; there are
no three.js lights. Everything lighting decides is decided in TRUE kilometres (`positionsKm`, `radiusKm`), never in
display space, so the terminator, the phases and the eclipses are the real ones in every scale preset: a moon drawn ten
times too large never casts a ten times larger shadow, and an eclipse happens exactly when the real geometry produces
one (the 8 April 2024 total solar eclipse, the 14 March 2025 total lunar eclipse; `lighting.test.ts`).

- Direction: each body is lit from the true direction of the Sun from its centre (`uSunKm`, the true Sun minus the
  true body, km). Parent-to-child directions are true in display space too (see Scale), so a planet's lit side faces
  the drawn Sun; for a moon the drawn Sun is off by at most a few degrees, the truth wins.
- Surface points: a fragment of the drawn sphere stands for the true surface point in the same direction from the
  body's centre (`normal * radiusKm`). A true shadow therefore lands on the same spot of the drawn globe however much
  the scale engine enlarges the body.
- Shading (`sunlightShader.ts`, `bodyFragmentShader`): Lambert `max(n.l, 0)` times the Sun's visible fraction, times
  `SUN_INTENSITY` (1.6), then ACES tone mapping. No distance falloff: Neptune is lit as brightly as Mercury (a 900x
  dimmer Neptune would be a black disc; legibility first). The night side keeps `NIGHT_LEVEL` (4.5 % of full
  sunlight, "starlight") and a faint cool rim (`NIGHT_RIM_COLOR`, stronger toward the limb), so a body's silhouette and
  surface stay readable on a projector while day and night stay unmistakable. Neither is physical; both are
  deliberate. A body with a `textures.night` map (Earth's city lights) shows it on the night side, faded in across a
  short twilight band; data decides, no body is special-cased.
- Shadows (eclipses) are analytic, no shadow maps: `sunVisibleFraction(point, sun, sunRadius, casters)` is the fraction
  of the Sun's disc visible from a point, from the exact lens area of the Sun's and each caster's angular discs
  (umbra 0, penumbra in between, annular eclipses leave a ring). Several casters multiply. Angles use the chord of unit
  vectors (`angleBetween`), exact for the milliradian sizes involved even in float32. The shader's `sunVisibility()`
  is a line-by-line port of it; keep the two in step (the TypeScript one is the tested one).
- Casters (`occluderCandidates`, once per body): a moon is shaded by its planet (lunar eclipse) and its siblings
  (Jupiter's moons shading each other); a planet by its moons (a solar eclipse seen from space). The Sun casts and
  receives nothing; planets never shade each other. `selectOccluders` (every frame) keeps at most `MAX_OCCLUDERS` (4)
  whose penumbra can reach the body right now, best aligned first, packed relative to the body's centre in true km
  (`uOccluders[k] = (x, y, z, radius)`). A caster that is not drawn (a hidden moon, `isBodyShown`) casts no shadow: a
  shadow without its caster reads as a bug.
- Uniforms: one `SunlightUniforms` object per body (`createSunlightUniforms`), rewritten by `updateSunlight(uniforms,
frame, index, sunIndex, casters, accept, alwaysLit)` in the body's `useFrame`. Materials share its uniform OBJECTS by
  reference (`createSunlitMaterial`, handed to R3F as a `<primitive>`): R3F's `<shaderMaterial uniforms>` prop copies
  each uniform into a new object, which would freeze scalar uniforms at mount.
- "Always lit" (`useLightingStore`: `alwaysLit`, `setAlwaysLit`; the HUD switch): honesty by default, convenience on
  request. Every body is then lit from the viewer (`0.35 + 0.65 * n.v`, still visibly round), with no night side and no
  shadows. Not persisted and not in the URL.
- Phases are a consequence, not a feature: the Moon (or any body) seen from anywhere shows exactly the half the Sun
  lights. `phaseAngle(positions, target, observer, sun)` and `illuminatedFraction(angle)` state it as numbers for the
  HUD and tests. Seasons likewise: the IAU spin axes (see Simulation) plus the true Sun direction give polar day and
  night on Earth and Uranus's pole pointed at the Sun, with no seasonal code.

Building on it:

- Anything that must be lit by the Sun (#12 rings, #23 small bodies, #35 spacecraft) includes `SUNLIGHT_PARS` in its
  fragment shader, shares the uniforms of the body it belongs to (or creates its own with `createSunlightUniforms` and
  calls `updateSunlight`), and evaluates `sunVisibility(p)` with `p` in that body's TRUE frame (body centre at the
  origin, true km, scene axes). Ring fragments are in the planet's frame already: `p = localDisplayPoint * radiusKm /
drawnRadiusKm` (rings keep true proportions, see Scale), and the planet's shadow on its rings comes out of the planet
  itself as a caster at the origin (`uOccluders` entry `(0, 0, 0, radiusKm)`).
- Ring shadows ON the planet (#12): add the rings as a transmittance term inside `sunVisibility`: intersect the ray
  from `p` toward `uSunKm` with the ring plane (the planet's equator: `spinAxis`), and multiply by `1 - ringAlpha(r)` for
  `innerRadiusKm <= r <= outerRadiusKm`; the ring's alpha strip is the texture to sample. Add the uniforms (plane
  normal, radii, alpha map) behind a define so bodies without rings pay nothing.
- #13 (rotation) needs nothing: a day is the spin under a fixed terminator.
...
- Scene: `<Canvas dpr={[1, 2]} gl={{ logarithmicDepthBuffer: true, antialias: true }} camera={{ near: 1e-5, far: 1e9, fov: 45 }}>`
  ...
  moon from clipping it) on a `#0b0d12` background. Lighting: no three.js lights at all; every body but the Sun is drawn
  by the sunlight model (see Lighting). The Sun is a `meshBasicMaterial` with `toneMapped={false}`; Bloom
  (postprocessing) arrives with `Effects` in Phase 6.
...
- Toggles: orbits, labels, moons, markers, "Always lit" (see Lighting). Hiding the moons never hides the focus: `isBodyShown(body, state)` (`src/store/sim.ts`)
...
- Body meshes: `bodies/BodyMesh.tsx` gets `{ body, index }`, a `<group>` positioned in `useFrame` via
  ...
  moons, 16 for estimated-radius moons (three geometries for all bodies). It owns the body's sunlight uniforms and
  rewrites them in the same `useFrame` (`updateSunlight`, see Lighting).
- Orbit lines: `bodies/OrbitLine.tsx` gets `{ body, index, parentIndex }` and owns a `BufferGeometry` with 258 points:
  ...
  A precessing orbit (the Moon) is resampled from `orbitAt(orbit, jd)` once it has turned `ORBIT_RESAMPLE_DEG` (0.05 deg)
  since the last sampling (`syncOrbitPrecession`), which also re-maps the display samples.
