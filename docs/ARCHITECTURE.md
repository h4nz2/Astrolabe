...
src/store/                   sim.ts, navigation.ts, scale.ts, lighting.ts, trails.ts, simSearch.ts (URL schema), urlSync.ts
...
off by default, as `orbitNames=true` while on; `frame=<id>` while a body is held still (#31). Defaults (overview,
...
- Page (`index.tsx`): `<UrlSync />`, then `scene/Scene.tsx` (Canvas + `SimFrameContext.Provider`, `ScaleSync`,
  `ReferenceFrameSync`, `SimClock`, `HoverCursor`, `Bodies`, `OrbitLines`, `Trails`, `Markers`, `Labels`, `CameraRig`, later `Effects`; then
