...
- Building on it: #17 moons (focus is how they are seen), #18 fly (clicks call `setFocus`; a fly profile can be
  requested through `focus(id, request)`), #24 compare ("Compare with…" in the card's action row, see Comparison),
...
- **Entry:** the focused body's card (`ui/BodyInfo.tsx`) has "Compare with…" in its action row, visible also while a
